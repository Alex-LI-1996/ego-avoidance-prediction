...
class Matrix_Loss(nn.Module):
    def __init__(self, kld_parm=None, cc_parm=None, sim_parm=None):
    ...
    def forward(self, s_map, gt):
        ...
        if self.cc_parm != 0:
            self.cc = 1 - cc(s_map, gt)

        if self.sim_parm != 0:
            self.sim = similarity(s_map, gt)

        loss = self.kld_parm * self.kld + self.cc_parm * self.cc + self.sim_parm * self.sim

        return [loss, self.kld, self.cc, self.sim]